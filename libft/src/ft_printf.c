...
/*   Updated: 2022/09/27 16:15:12 by yeongo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	convert_format(const char format, va_list ap, int *result)
{
	if (format == '\0')
		...
		return (ft_putnbr_base_count(va_arg(ap, unsigned int), 16, format, result));
	else if (format == 'u')
		return (ft_putnbr_base_count(va_arg(ap, unsigned int), 10, format, result));
	else if (format == 'p')
	{
